...
def build_Mr_apply(
    ...
    *,
    ...
    def init_fn(positions, *, extra_capacity_override=None, **kwargs):
        ...
        # For shear-capable boxes in fractional coordinates, optionally pre-allocate
        # the neighbor list at a bounded worst-case shear corner. This is a safety
        # heuristic for bounded remapped shear; when unsupported (e.g., box_fn
        # signature cannot accept per-plane gamma kwargs) we keep the current box.
        if box_fn is not None and fractional_coordinates and neighbor_box is not None:
            ...
            neighbor_kwargs["box"] = neighbor_box  # always override for shearing
