...
step and stores it in `state.stress` using the event-driven impulse estimator:

$$
\sigma = -\frac{1}{V\,dt}\sum_{\text{collisions}} (\Delta v_i\,dt) \otimes r_c,
\quad r_c = \sigma\,\hat n,\quad
\Delta v_i = v_i^* - v_i.
$$

Here $\Delta v_i$ is the collision-induced jump in the **peculiar**
velocity of one particle at contact (a “kick”); for equal masses the relative
jump $\Delta v_{\mathrm{rel}} = (v_i^* - v_j^*) - (v_i - v_j)$ satisfies
$\Delta v_{\mathrm{rel}} = 2\,\Delta v_i$, so if you only track the relative
change you should use $0.5\,\Delta v_{\mathrm{rel}}$. Accumulating as
$(\Delta v \otimes r)$ makes the component convention explicit, e.g.
$\sigma_{xy} \propto \Delta v_x \, \Delta r_y$. This is the hard-sphere
constraint contribution only (no ideal term).
